...
### D17 — Cash modelled as a running position, not a per-shift register float
- **Date:** 2026-05-15
- **Context:** The owner explained Qave's actual cash workflow: daily cash sales accumulate in a safe, cash expenses are paid out of the day's takings or the safe, and the till is not reset to a fixed float each shift. The original `cash_float_start / cash_float_end / over_short` model assumed a classic register float and produced meaningless results — e.g. May 14 showed `over_short = −298` purely as a formula artifact (that day had 186 cash sales against 1,889.90 of cash expenses paid from the safe).
- **Decision:** Track cash as a single **running cash-on-hand position** (till + safe combined). New `cash_events` table: a `count` event sets the baseline (opening balance / physical recount / "zero out"), a `withdrawal` event deducts cash taken out. `v_cash_position` computes the balance = latest count + cash sales − cash expenses − withdrawals dated after that count. Cash sales and cash expenses are **derived** from `closings`/`expenses`, never duplicated. Opening balance seeded at AED 165.50 on 2026-05-15 (owner-supplied).
  - The misleading `over_short`-based "cash discrepancy" alert was removed from the dashboard. The `cash_float_*` columns and `over_short` generated column are left in place (not dropped) but are **superseded** — flagged for a cleanup pass.
  - Count semantics: a count's amount is the cash on hand *as of the end of its `event_date`*; the running balance accrues activity dated strictly after. v1 has no scheduled reconciliation, so the balance drifts from physical reality until the owner records a recount — that's the intended correction mechanism.
- **Rationale:** Matches how the café actually runs; reuses data the app already captures; minimal schema (one table); the recount path doubles as drift correction. One number (not a till/safe split) is the meaningful control figure for v1.
- **Alternatives:** keep patching the float formula (rejected — wrong model); track till and safe as separate balances (rejected for v1 — needs the owner to log every safe transfer); full cash ledger with every movement as a row (rejected — sales/expenses are already stored elsewhere; duplicating them invites drift).
- **Decided by:** Owner (workflow + "add it to the dash") + Claude (model + implementation)
